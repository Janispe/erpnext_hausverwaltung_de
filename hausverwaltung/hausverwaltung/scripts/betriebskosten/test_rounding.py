...
class TestBKRounding(TestCase):
	...
	def test_tenant_only_rounding_preserves_unrounded_apartment_shares(self):
		result = round_money_allocations(self.entries, ROUNDING_METHOD_TENANT_ONLY)

		self.assertEqual(result, dict(self.entries))
		self.assertEqual(sum(result.values()), Decimal("99.999"))
